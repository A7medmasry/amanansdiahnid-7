  ...
  <div>
    <div v-if="ads[0]">
      <div class="header">
        ...
        <p style="font-size: 20px">inmueebles en venta</p>
      </div>
            ...
            <template v-slot:activator="{ on }">
              <div style="position: relative;  width: 100%;">
                <input
                  v-model="search"
                  type="text"
                  placeholder="Buscar en categoría de inmueebles en venta"
                  class="search-ads"
              ...
              <v-row no-gutters>
                ...
                <v-col cols="8">
                  <router-link :to="$route.path + '/' + ad.id">
                    <div class="titleAd" :class="dark ? 'titleAd--dark' : ''">{{ad.title.length > 46 ? ad.title.substring(0,45)+"...": ad.title}}</div>
                  </router-link>
                  <div
                    class="priceAd"
...
export default {
  ...
  async asyncData({ route }) {
    let adsTotal = await db.collection("inmueebles-en-venta").get();
    var ads = [];
  ...
  head() {
    ...
    return {
      title: 'inmueebles en venta',
      titleTemplate: '%s ← Klkrd!',
      meta: [
        {
          vmid: 'description',
          name: 'description',
          content: 'Vende casa, apartamento, villas solar y local comercial en República Dominicana'
        }
      ]
    }
  },
  beforeMount() {
    db.collection("inmueebles-en-venta")
      .get()
      .then(querySnapshot => {
