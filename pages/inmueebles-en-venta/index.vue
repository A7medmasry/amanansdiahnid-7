          ...
          <v-col cols="12" v-for="(ad, index) in filterSearch" :key="ad.id">
            <v-card class="mb-4" raised v-scroll-reveal.reset="{ delay: 800 }">
              <v-row no-gutters>
                ...
                <v-col cols="8">
                  <nuxt-link :to="'/inmueebles-en-venta/' + ad.id">
                    <div
                      class="titleAd"
...
export default {
  ...
  async asyncData({ app }) {
    let adsTotal = await app.$fireStore.collection("inmueebles-en-venta").get();
    var ads = [];
      ...
      meta: [
        {
          ...
          name: "description",
          content:
            "Vende casa, apartamento, villas solar y local comercial en República Dominicana"
        }
      ]
    };
  },
  ...
  beforeMount() {
    this.$fireStore
      .collection("inmueebles-en-venta")
      .get()
      .then(querySnapshot => {
